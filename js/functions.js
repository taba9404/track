...
function displayNearbyStations() {
    ...
    for (let key of Object.keys(nearbyStations)) {
        ...
        cardHTML += '<p>‹——£: ' + getOptimizedDistance(distance) + '</p>';
        cardHTML += '<ul>';
        cardHTML += '</ul>';
        cardHTML += '</div>';
        cardHTML += '<div class="card-action">';
        cardHTML += '<a href="https://www.google.com/search?q=' + station.name + '‰w" target="_blank" rel=""noopener"">Google‚ÅŒŸõ</a>';
        cardHTML += '</div>';
        cardHTML += '</div>';
...
function getOptimizedDistance(distance) {
    ...
    if (roundedDist < 1) {
        return "–ñ" + (roundedDist * 1000) + "m";
    } else {
        return "–ñ" + roundedDist + "km";
    }
}
