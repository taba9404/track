...
function displayNearbyStations() {
    ...
    for (let key of Object.keys(nearbyStations)) {
        ...
        cardHTML += '<p>KYORI: ' + getOptimizedDistance(distance) + '</p>';
        cardHTML += '<ul>';
        cardHTML += '</ul>';
        cardHTML += '</div>';
        cardHTML += '<div class="card-action">';
        cardHTML += '<a href="https://www.google.com/search?q=' + station.name + '駅" target="_blank" rel=""noopener"">Google Search</a>';
        cardHTML += '</div>';
        cardHTML += '</div>';
...
(function init() {
    doRequest("GET", "https://taba9404.github.io/track/data/json/stations.json");
})();
